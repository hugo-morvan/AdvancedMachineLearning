...
get_probs = function(myHMM, obs){
  ...
  return(list("filtered" = filtered, "smoothed" = smoothed, "most_probable_path" = most_probable_path))
}
...
get_accuracies = function(sim, get_probs_result){
  ...
  return(list("filtering_accuracy" = filtering_accuracy, 
              "smoothing_accuracy"= filtering_accuracy, 
...
The smoothed distribution should also be morea accurate than the most probable path because 
...
plot(filt300_entropies, type="l")
abline(a=mean300, b = 0)
```

It does not seems like the entropy reduces the later in time, meaning that we do not know better where the robot is over time.
...
legend("topright", legend=c("true state @ t=100", "smoothed state prob. @ t=100", "state prob @ t = 101"), 
